...
function loadProjectFile(data, success) {
    ...
    for (i = 0; i < templates.length; i++) {
        ...
        for (keyID = 0; keyID < keys.length; keyID++) {
            ...
            if (keys[keyID].getAttribute("richText") == "true") tempKey.richText = true;
            else tempKey.richText = false;
...
function initContextMenus() {
	...
	$(function(){
    $('.hasContentsContextMenu').on('taphold', function(e) {
        e.preventDefault();
        $('.hasContentsContextMenu').contextMenu();
    });    	
    	$.contextMenu({
		zIndex: 1000,
		selector: '.hasContentsContextMenu',
...
function parseMDField (accessKey, field) {
	...
	if ( accessKey == "" || reader.project.templates[accessKey].richText == true ) {
		...
		return $(field).append(htmlText).text();
	}
}
...
function setGUI(forceLoad) {
    ...
    if (loadViewFile || (forceLoad && guiMode == 0)) {
        ...
        $(Object.keys(view.layers)).each(function(index, layerID) {
            var layer = view.layers[layerID];
            var sWidth = 0;
            var svgLayer = reader.canvas.svg.group(reader.canvas.layerGroup, layer.id + "_group", {
                class: 'layerContextMenu',
                strokeWidth: sWidth * (1 / reader.zoom.cur),
                fill: layer.color,
                stroke: layer.color,
                fillOpacity: 0.0
            });
            ...
            if (tooltipDiv.length > 0) $(svgLayer).tooltip({
                ...
                position: 'center left',
                onBeforeShow: function(tt, pos) {
                    $('#canvasTooltip .tooltipContent').html(tooltipDiv);
                    $(tt.target).tooltip().getConf().offset[1] = Math.round(reader.pageX - pos.left) + 64;
                ...
                onShow: function(tt, pos) {
                    ...
                    this.getTip().draggable({
                        containment: '#canvas'
                    });
                },
                onHide: function(tt, pos) {
                    try {
                        this.getTip().draggable("destroy");
                    } catch (e) {}
                }
            });
            if ($('.startitem').attr("href") == location.hash) {
	    		$(".startitem").one("vclick", function( e) { 
	    		e.preventDefault();
				});
			} else {
				$(".startitem").on("vclick", function( e) { 
	    		return true;
				});
			}
            if (screen.width <= 1024) {
            ...
            $(svgLayer).hover(function(ev, source) {
            ...
            if (scaledPath != '') reader.canvas.svg.path(svgLayer, scaledPath + ' Z', {
                fillRule: 'evenodd'
            });
        });
    }
