...
function displayLightTable() {
    ...
    function addFrame(item) {
        ...
        if (item == null) {
            ...
            $('#' + id + "_content").bind('mouseup', function(e) {
                window.syncTableToModel();
            });
            var title = '';
...
function calcImageScales() {
    ...
    if (screen.width <= 1024) {
      ...
      $('#canvasImageGroup').panzoom({
							disablePan: true,
							contain: false,
							minScale: 0.1,
							maxScale: reader.zoom.mobile,
						});
      
		 $('#canvasImageGroup').on("panzoomstart", function( e, view, scale, panzoom ) {
		 	$('svg').css({top:'55px'});
		   	$('svg').css({width:'100%'});
		   	$('svg').css({height:'100%'});
		...
		$('#canvasImageGroup').on("panzoomend", function( e, view, panzoom ) {
			getLayerBounds();
			loadHiResIfNeeded(view);
			findHigherRes(view);
			if ($('.startitem').attr("href") == location.hash) {
	    		$("a.startitem").one("vclick", function( e) { 
	    		console.log('same');
	    		e.preventDefault();
				});
			} else if ($('.startitem').attr("href") != location.hash) {
				$("a.startitem").on("vclick", function( e) { 
	    		return true;
				});
			}
		...
		$(window).on('popstate', function() {			
			$('#canvasImageGroup').panzoom('destroy');
			$('svg').css({top:'0px'});			
		});
	...
	$(function(){
		...
		$('[id$=_group]').on('taphold', function(e) {
			...
			$('#layer-to-lita').one('click', 'vclick', function(e) {
				e.preventDefault();
				addLayerToLightTable(layerID);
			});
							
			$('#layer-tracking').one('click', 'vclick', function(e) {
				e.preventDefault();
    ...
    if (loadViewFile || (forceLoad && guiMode == 0)) {
		...
		*/
		...
		$(view.sortedLayers).each(function(index, layer) {
			...
			if ($('.startitem').attr("href") == location.hash) {
	    		$("a.startitem").one("vclick", "click", function( e) { 
	    			console.log('same');
					e.preventDefault();
				});
			} else if ($('.startitem').attr("href") != location.hash) {
				$("a.startitem").on("vclick", "click", function( e) { 
	    			return true;
				});
			}
			if (screen.width <= 1024) {
